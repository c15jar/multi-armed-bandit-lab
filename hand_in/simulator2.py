...
class Simulator:
    ...
    def randomize(self):
        ...
        self.expected_rewards_approx = [
            1 + (random() / 2) for _ in range(4)
        ]
        self.expected_rewards_approx.append(-5)
        self.expected_rewards_approx.append(-10)
        shuffle(self.expected_rewards_approx)
        for _ in range(10000):
            for arm_index in range(6):
                acc_rewards[arm_index] = acc_rewards[arm_index] + generate_reward(arm_index, self.expected_rewards_approx)
    ...
    def simulate(self, bandit):
        for _ in range(1000):
            arm = bandit.run()
            reward = generate_reward(bandit.arms.index(arm), self.expected_rewards_approx)
